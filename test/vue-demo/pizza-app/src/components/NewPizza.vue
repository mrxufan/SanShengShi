	...
	export default{
		...
		methods:{
			addMenuItem(){
				...
				// fetch('https://wd5866137243flqsvb.wilddogio.com/menu.json',{
				// 	method:'POST',
				// 	headers:{
				// 		'Content-type':'application/json'
				// 	},
				// 	body:JSON.stringify(data)
				// }).then(res=>res.json()).then(data=>this.$router.push({name:'menuLink'})).catch(err=>console.log(error))

				// this.http.post('menu.json',data).then(res=>this.$router.push({name:'menuLink'}))

				// 数据同步到vuex中
				this.http.post('menu.json',data).then(res=>this.$store.commit('pushToMenuItems',data))
			}
		}
	}
</script>